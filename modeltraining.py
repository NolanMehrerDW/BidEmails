...
def get_model_predictions(email_body):
    ...
    # If the model doesn't extract the date, use regex as fallback
    if not bid_due_date:
...
def format_date_to_mmddyyyy(date_str):
    ...
    try:
        # Try to parse dates in both formats
        date_obj = datetime.strptime(date_str, "%B %d, %Y")  # "Month DD, YYYY"
    except ValueError:
        try:
            date_obj = datetime.strptime(date_str, "%m/%d/%Y")  # "MM/DD/YYYY"
        except ValueError:
            return date_str  # If it can't be parsed, return as is
